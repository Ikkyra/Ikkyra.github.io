  <?php 
    ...
    if (isset($_GET['search'])) {
      $search = $_GET['search'];
      $sql = mysqli_query($conn, "SELECT * FROM booking WHERE nama LIKE '%$search%'");
      $booking = [];
  ...
  <body>
    ...
    <header>
      ...
      <search>
      <form action="" method="GET" class="search-bar-booking">
        <input type="text" name="search" placeholder="Search Booking"
          class="search-input-booking" />
        <button type="submit" class="search-button-Booking" >
          Search
        </button>
      </form>
      </search>
